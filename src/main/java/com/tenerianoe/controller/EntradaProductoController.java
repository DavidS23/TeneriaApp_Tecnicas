...
public class EntradaProductoController implements Serializable {
    ...
    public void init() {
        ...
        listaDetalles = detalleEJB.findAll();

    }

    public void insertarFacturaProcedimiento() {
        if (factura.getIdFactura() == 0) {
            facturaEJB.insertarFacturaProcedimiento(factura);
            factura.setIdFactura(facturas.size() + 1);

            detalleEJB.insertarDetalleFacturaProcedimiento(detalleFactura, factura, productoSeleccionado);
            listaDetalles = detalleEJB.findAll();
            detalleFactura = listaDetalles.get(listaDetalles.size() - 1);
            ...
            detalleFactura = listaDetalles.get(listaDetalles.size() - 1);
            listaDetalleFactura.add(detalleFactura);
            detalleEJB.calcularTotalProcedimiento(factura);
    ...
    public void facturar() {
        ...
        for (DetalleFactura item : listaDetalleFactura) {
            totalFacturaCompra.add(item.getTotalCompra());
